...
Create a new file with the SVM username and admin password, and save it as svm_secret.yml. A sample svm_secret.yml file is included in the fsx folder.
...
cd into the `files/mysql` folder in your cloned git repository. Review the file `mysql-secrets.yml` file, and execute the following command to create the secret.
