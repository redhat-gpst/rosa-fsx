...
In order to use the `oc` command, you must first login.  
...
[source, bash]
----
rosa@bastion fsx]$ oc login https://api.rosa-6slmk.qkm6.p1.openshiftapps.com:6443
...
[rosa@bastion fsx]$ oc login --token=sha256~Bt3c-m2dTmmHqGlo-xxxxxxxxxxxxxxxxxxxxxxxx --server=https://api.rosa-6slmk.qkm6.p1.openshiftapps.com:6443
...
Edit the file `backend-ontap-nas.yaml`. 
...
cd into the `files/mysql` folder in your cloned git repository. Review the file `mysql-secrets.yaml` file, and execute the following command to create the secret.
...
Next we will deploy MySQL StatefulSet application on the ROSA cluster. Open `mysql/mysql-statefulset.yaml` and review the details –metadata, replicas, and storageclass name. Then, execute the following command. 
